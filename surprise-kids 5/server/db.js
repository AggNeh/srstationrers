...
const products = {
  async query({ category, search, sort = 'newest', featured, page = 1, limit = 24 } = {}) {
    const filter = {};
    if (category) {
  ...
  async update(id, patch) {
    ...
    for (const f of fields) {
      ...
      else if (f === 'inStock' || f === 'featured') set[f] = !!patch[f];
      else set[f] = patch[f];
    }
    // Re-normalize image/images so they stay in sync.
