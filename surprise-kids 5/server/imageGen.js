 ...
 *
 * Output destination:
 *   - If object storage is configured (STORAGE_* env vars), the SVG is
 *     uploaded to the bucket under products/<slug>.svg and a public URL
 *     is returned.
 *   - Otherwise it falls back to writing /public/images/products/<slug>.svg
 *     for local dev (this directory is ephemeral on Render's free tier).
 */
...
/**
 * Write the SVG to durable storage (R2/S3) or local disk fallback.
 * @returns {Promise<string>} the public URL of the image
 */
async function writeProductImage(slug, name, cat) {
  const svg = productSvg(name, cat);
  const key = `products/${slug}.svg`;

  if (storage.isConfigured()) {
    return storage.uploadBuffer(key, Buffer.from(svg, 'utf8'), 'image/svg+xml');
  }
