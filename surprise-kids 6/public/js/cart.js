...
function renderItems() {
  ...
  $('#items').innerHTML = cart.map((it) => `
    <div class="cart-item" data-id="${it.productId}">
      <img src="${esc(it.image || PLACEHOLDER_IMG)}" alt="${esc(it.name)}" ${IMG_FALLBACK}>
      <div>
...
async function checkout() {
  ...
  try {
    ...
    // Save this order's stub (with its private edit token) so the customer
    // can find / edit / cancel it later from the My Orders page.
    try {
      const KEY = 'sr_orders_v1';
      const arr = JSON.parse(localStorage.getItem(KEY)) || [];
      arr.unshift({ id: data.order.id, reference: data.order.reference, token: data.order.token });
    ...
    $('#cart-root').innerHTML = `
      <div class="empty">
        ...
        <a class="btn btn-ghost" href="/order/${esc(data.order.reference)}?id=${data.order.id}&token=${esc(data.order.token)}" style="margin-left:8px">View & edit this order</a>
        <a class="btn btn-ghost" href="/shop" style="margin-left:8px">Continue shopping</a>
      </div>`;
  } catch (e) {
