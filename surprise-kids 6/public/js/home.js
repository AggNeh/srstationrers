...
async function loadComingSoon() {
  const section = $('#coming-soon-section');
  const strip = $('#coming-soon-strip');
  if (!section || !strip) return;
  try {
    const data = await fetch(`${API}/products?comingSoon=true&limit=4&sort=newest`).then(r => r.json());
    ...
    strip.innerHTML = items.map((p, i) => {
      const cover = imgOf(p);
      return `
        <a class="cs-mini reveal" style="animation-delay:${i * 0.04}s" href="/coming-soon">
          <div class="cs-mini-media"><img src="${esc(cover)}" alt="${esc(p.name)}" loading="lazy" ${IMG_FALLBACK}><span class="cs-badge">Soon</span></div>
          <div class="cs-mini-name">${esc(p.name)}</div>
        </a>`;
    }).join('');
