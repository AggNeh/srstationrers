      ...
      <div>
        ...
        ${editable ? `
          <div class="add-item-box">
            <button class="btn btn-ghost btn-sm" id="add-toggle">+ Add another item</button>
            <div id="add-panel" class="add-panel" hidden>
              <input id="add-search" placeholder="Search products to add…" autocomplete="off">
...
/* Inline "add another item" search — adds straight into THIS order's items
   (fixes the old behaviour where adding went to the cart and started a new
   order on checkout). */
function wireAddItem() {
  ...
  toggle.onclick = () => {
    ...
    if (!panel.hidden) { search.focus(); runSearch(''); }
  };

  let timer;
  search.addEventListener('input', () => {
    clearTimeout(timer);
    timer = setTimeout(() => runSearch(search.value.trim()), 200);
  });

  async function runSearch(q) {
    results.innerHTML = '<p class="muted" style="padding:8px">Searching…</p>';
    try {
      const r = await fetch(`${API}/products?limit=8&sort=newest${q ? `&search=${encodeURIComponent(q)}` : ''}`);
      const data = await r.json();
      const items = (data.items || []).filter(p => p.inStock);
      if (!items.length) { results.innerHTML = '<p class="muted" style="padding:8px">No products found.</p>'; return; }
      results.innerHTML = items.map(p => `
        <button class="add-result" data-add='${esc(JSON.stringify({ id: p.id, name: p.name, price: p.price, image: imgOf(p) }))}'>
          <img src="${esc(imgOf(p))}" alt="" ${IMG_FALLBACK}>
          <span class="ar-name">${esc(p.name)}</span>
          <span class="ar-price">${money(p.price)}</span>
        </button>`).join('');
      results.querySelectorAll('[data-add]').forEach(b => b.onclick = () => {
        const p = JSON.parse(b.getAttribute('data-add'));
        addItemToOrder(p);
      });
    } catch (e) { results.innerHTML = '<p class="muted" style="padding:8px">Search failed.</p>'; }
  }
}

function addItemToOrder(p) {
  const existing = CURRENT.items.find(it => Number(it.productId) === Number(p.id));
  if (existing) existing.qty += 1;
  else CURRENT.items.push({ productId: p.id, name: p.name, price: p.price, qty: 1 });
  toast(`Added “${p.name}”`);
  renderItems();
}
