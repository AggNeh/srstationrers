/* admin.js — management dashboard
   NOTE: the passcode gate here is a lightweight UI lock for the demo, not real
   server-side authentication. Add proper auth before any public deployment. */

const ADMIN_PASS = 'admin123';
const AUTH_KEY = 'sk_admin_ok';
...
function initAuth() {
  if (sessionStorage.getItem(AUTH_KEY) === '1') return showShell();
  $('#login-btn').onclick = tryLogin;
  $('#pass').addEventListener('keydown', (e) => { if (e.key === 'Enter') tryLogin(); });
}
function tryLogin() {
  if ($('#pass').value === ADMIN_PASS) { sessionStorage.setItem(AUTH_KEY, '1'); showShell(); }
...
function categoryForm(c, onSaved) {
  const isEdit = !!c;
  const initial = c || { name: '', emoji: '🎁', color: '#FF5A6A', accent: '', position: '' };
